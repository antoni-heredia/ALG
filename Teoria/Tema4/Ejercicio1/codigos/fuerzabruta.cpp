...
void FuerzaBruta(const vector<int> & conjunto, int sumaObjetivo, int  posActual, vector<int>  solucion, vector<vector<int>> &soluciones);
...
int main(int argc, char* argv[]){
	...
	tini = clock();
	FuerzaBruta(conjunto,sumaObjetivo,0,solucion,soluciones);
	tfin = clock();
...
void FuerzaBruta(const vector<int> & conjunto, int sumaObjetivo, int  posActual, vector<int>  solucion, vector<vector<int>> &soluciones){
	...
	if(posActual == conjunto.size()){
		...
		while(posActual < conjunto.size()){
			...
			posActual++;
			FuerzaBruta(conjunto,sumaObjetivo,posActual,solucion,soluciones);
			solucion.pop_back();
