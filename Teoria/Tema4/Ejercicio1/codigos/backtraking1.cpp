...
void BackTraking(const vector<int> & conjunto, int sumaObjetivo, int  posActual, vector<int>  solucion, vector<vector<int>> &soluciones);
...
int main(int argc, char* argv[]){
	...
	tini = clock();
	BackTraking(conjunto,sumaObjetivo,0,solucion,soluciones);
	tfin = clock();
...
void BackTraking(const vector<int> & conjunto, int sumaObjetivo, int  posActual, vector<int>  solucion, vector<vector<int>> &soluciones){
	...
	if(posActual == conjunto.size()){
		...
		while(posActual < conjunto.size() && conjunto[posActual] <= sumaObjetivo){
			...
			posActual++;
			BackTraking(conjunto,sumaObjetivo,posActual,solucion,soluciones);
			solucion.pop_back();
